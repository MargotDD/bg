...
interface AppContextType {
  ...
  createCompany: (name: string, description: string, currency: string, logoUrl?: string) => Promise<{ company: Company; inviteCode?: string }>;
  ...
  createCompanyInviteCode: (companyId?: string) => Promise<string>;
  joinCompanyByCode: (code: string) => Promise<void>;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    Promise.all([api('/api/state'), api('/api/me')]).then(([data, me]) => {
      ...
      setState(prev => {
        ...
        const filteredCompanies = base.companies.filter(c => allowed.has(c.id));
        const filtered = {
          ...base,
          ...
          activeCompanyId: companyIds.find(id => filteredCompanies.some(c => c.id === id)) || ''
        };
        return filtered;
      });
  ...
  const createCompany = useCallback(async (name: string, description: string, currency: string, logoUrl?: string) => {
    ...
    try {
      ...
      const invite = await api('/api/company/invite-code',{method:'POST',body:JSON.stringify({company:newCompany,companyState})});
      inviteCode=invite.code;
    } catch { /* company creation still succeeds; invite can be generated later */ }
    return { company:newCompany, inviteCode };
  }, [state, currentUser]);
  ...
  const value = useMemo(() => ({
    ...
    switchCompany,
    createCompany,
    createCompanyInviteCode, joinCompanyByCode,
    ...
    updateUserRole,
    updateCurrentUserProfile, companyAlerts, markCompanyAlertRead,
    finances,
    createSale,
    returnSale,
    createProduct,
