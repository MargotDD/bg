...
export async function api(path:string, options:RequestInit={}) {
  ...
  if (path === '/api/company/invite-code' && method === 'POST') {
    const body = JSON.parse(String(options.body || '{}'));
    ...
    if (company.ownerId === u.id) {
      await rest('/rest/v1/company_members', {
        method:'POST',
        headers:{Prefer:'resolution=merge-duplicates'},
        body:JSON.stringify({company_id:company.id,user_id:u.id,role:'Owner',permissions:{
          create_edit_delete_products:true,make_sales:true,view_customers:true,create_purchases:true,
          view_finances:true,edit_financial_records:true,delete_financial_records:true,manage_members:true,
          manage_announcements:true,manage_goals:true,manage_plans:true,create_admin_spending:true,
          edit_delete_admin_spending:true,view_sensitive_finance:true,create_sales:true,process_returns:true,
          create_expenses:true,manage_customers:true,restrict_customers:true,manage_team:true,
          adjust_finances:true,view_activity_logs:true
        }})
      });
    }
    const rawResult = await rpc('create_simple_company_code', {
      p_company_id: company.id,
      p_company: company,
      p_company_state: body.companyState || await extractCompanyStateFromStored(company.id)
    });
    const result = Array.isArray(rawResult) ? rawResult[0] : rawResult;
    if (!result?.code) throw new Error('Could not create company code. Apply the latest supabase/schema.sql.');
    return {code:result.code};
  }
  if (path === '/api/company/join-code' && method === 'POST') {
    const body = JSON.parse(String(options.body || '{}'));
    const code = String(body.code || '').trim().toUpperCase();
    if (!code) throw new Error('Enter a company code.');
    const rawResult = await rpc('join_company_by_code', {p_code:code});
    const result = Array.isArray(rawResult) ? rawResult[0] : rawResult;
    return {ok:true,state:result?.state,company:result?.company};
  }
