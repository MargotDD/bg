  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/40 backdrop-blur-xs animate-in fade-in duration-150">
      ...
      >
        ...
        {inviteLink && <div className="mt-4 rounded-2xl bg-[#FFF0F6] border border-rose-100 p-3"><p className="text-xs font-bold text-stone-700 mb-2">Company created! Send this invitation link:</p><div className="flex gap-2"><input readOnly value={inviteLink} className="flex-1 min-w-0 rounded-xl border border-rose-200 bg-white px-3 py-2 text-[11px]"/><button type="button" onClick={async()=>{await navigator.clipboard?.writeText(inviteLink);setCopied(true);setTimeout(()=>setCopied(false),1500)}} className="rounded-xl bg-rose-600 text-white px-3">{copied?<Check className="w-4 h-4"/>:<Copy className="w-4 h-4"/>}</button></div></div>}
