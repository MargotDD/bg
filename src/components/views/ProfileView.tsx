...
export const ProfileView: React.FC<{ onCreateCompany?: () => void }> = ({ onCreateCompany }) => {
  const {
    currentUser, currentCompany, updateCurrentUserProfile, createInviteLink, state,
  ...
  return <div className="space-y-5">
    ...
    {!hasCompany ? <div className="rounded-3xl bg-white/90 border border-[#F9CAD4] p-6 shadow-sm"><div className="flex items-center gap-3"><div className="w-11 h-11 rounded-2xl bg-[#FFE8F0] flex items-center justify-center text-[#D85D78]"><Building2 className="w-5 h-5"/></div><div className="flex-1"><h3 className="font-extrabold text-[#3E2027]">Company</h3><p className="text-xs text-[#9E6775] mt-1">No company yet. This section stays empty until you create or join one.</p></div><button onClick={onCreateCompany} className="rounded-2xl bg-[#fb429c] text-white px-4 py-2.5 text-xs font-extrabold">Create company</button></div></div> :
    <div className="rounded-3xl bg-white/90 border border-[#F9CAD4] shadow-sm overflow-hidden">
      ...
      {tab==='company' ? <div className="p-5 space-y-4">
        ...
        <div className="rounded-2xl border border-[#F9CAD4] bg-[#FFF9FC] p-4 flex flex-col sm:flex-row sm:items-center gap-3 justify-between"><div><div className="font-extrabold text-sm text-[#3E2027]">Invitation</div><div className="text-xs text-[#9E6775]">Only real Owner/Admin accounts can create new invitations.</div></div>{isRealAdmin&&<button onClick={makeInvite} className="rounded-2xl bg-[#FFF0F6] border border-[#F9CAD4] text-[#A7445C] px-4 py-2.5 text-xs font-extrabold"><Link2 className="inline w-3.5 h-3.5 mr-1"/> New invitation</button>}</div>
        {invite&&<div className="flex gap-2"><input readOnly value={invite} className="flex-1 rounded-xl border border-[#F9CAD4] px-3 py-2 text-xs"/><button onClick={async()=>{await navigator.clipboard?.writeText(invite);setCopied(true);setTimeout(()=>setCopied(false),1500)}} className="rounded-xl bg-[#fb429c] text-white px-3">{copied?<Check className="w-4 h-4"/>:<Copy className="w-4 h-4"/>}</button></div>}
