    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-white p-6 rounded-3xl border border-rose-100 shadow-xs space-y-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-bold text-stone-900 text-sm">Invite people to this company</h3>
            <p className="text-xs text-stone-500">Create a shareable link. The invited person must have a Business Girls account.</p>
          </div>
        </div>
        {can('manage_team') && <>
          <button disabled={inviteBusy} onClick={async () => {
            ...
            try {
              const result = await api('/api/invites', {method:'POST', body: JSON.stringify({company: currentCompany, state})});
              setInviteLink(result.link);
              await navigator.clipboard?.writeText(result.link);
            } catch (e:any) { setInviteError(e?.message || 'Could not create invite.'); }
            finally { setInviteBusy(false); }
          }} className="px-4 py-2.5 rounded-2xl bg-rose-600 text-white text-xs font-bold hover:bg-rose-700 disabled:opacity-60">
            {inviteBusy ? 'Creating link…' : 'Create invitation link'}
          </button>
          {inviteLink && <div className="flex flex-col sm:flex-row gap-2">
            <input readOnly value={inviteLink} className="flex-1 px-3 py-2 rounded-xl border border-rose-200 bg-rose-50/30 text-xs" />
            <button onClick={() => navigator.clipboard?.writeText(inviteLink)} className="px-3 py-2 rounded-xl border border-rose-200 text-rose-700 text-xs font-bold">Copy</button>
          </div>}
